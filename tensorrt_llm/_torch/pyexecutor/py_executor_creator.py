        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                if mapping.enable_attention_dp:
                    ...
                    logger.info(f"Setting DYN_KVBM_LEADER_ZMQ_PUB_PORT to {pub_port}, DYN_KVBM_LEADER_ZMQ_ACK_PORT to {ack_port} for MPI rank {mpi_rank()}")
                    os.environ["DYN_KVBM_LEADER_ZMQ_PUB_PORT"] = str(pub_port)
                    os.environ["DYN_KVBM_LEADER_ZMQ_ACK_PORT"] = str(ack_port)
                    

                    if os.environ.get("DYN_KVBM_TRTLLM_ZMQ_PORT") is not None:
                        trtllm_port = int(os.environ.get("DYN_KVBM_TRTLLM_ZMQ_PORT")) + mpi_rank()
                        os.environ["DYN_KVBM_TRTLLM_ZMQ_PORT"] = str(trtllm_port)
                        logger.info(f"Setting DYN_KVBM_TRTLLM_ZMQ_PORT to {trtllm_port} for MPI rank {mpi_rank()}")
